    ...
    <div class="py-8">
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow rounded-lg overflow-hidden bg-white">
                @can("create-placetype")
                    <div class="flex">
                        <a href="{{route("admin.placeTypes.create")}}"
                           class="p-3 rounded-lg bg-blue-600
                           hover:bg-blue-800 text-white  mt-9 mr-4 ">
                            ثبت نوع مکان جدید
                ...
                <table class="min-w-full leading-normal">
                    ...
                    @foreach($placeTypes as $placeType)
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">
                                    <img src="{{env("app_url").":8000/".$placeType->icon}}" class="w-10 h-10 rounded" alt="{{$placeType->slug}}">
                                </p>
                            </td>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <div class="flex">
                                    @can("edit-placetype")
                                        <a href="{{route("admin.placeTypes.edit",$placeType->id)}}">
                                    ...
                                    @endcan
                                    @can("destroy-placetype")
                                        <form action="{{route("admin.placeTypes.destroy",$placeType->id)}}" method="post">
                                            @method("DELETE")
                                            @csrf
                                            <button type="submit">
                ...
                {{$placeTypes->appends(['search'=>request('search')])->links()}}
            </div>
        </div>
    </div>
</x-adminlayoutpanel>
